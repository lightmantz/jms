<?php
// modules/admin/pages/create-submission.php - Create New Submission with PDF Upload
if (!defined('SITE_URL')) {
...
$stmt = $db->query("SELECT id, full_name, email FROM users WHERE is_active = 1 ORDER BY full_name");
    ...
    if (empty($title) || empty($author_id)) {
        ...
        try {
            ...
            $result = $stmt->execute([
                ...
                $abstract, 
                $author_id, 
                $author_id, // corresponding_author_id
                $article_type, 
                $status, 
                $funding_source, 
                $acknowledgments, 
            ...
            // If pdf_file column doesn't exist, try without it
        ...
        if ($result) {
            ...
            if ($status == 'submitted') {
                $stmt = $db->prepare("UPDATE manuscripts SET submitted_at = NOW() WHERE id = ?");
                ...
                foreach ($editors as $editor) {
                    createNotification(
                        ...
                        SITE_URL . '/admin?action=manuscript&id=' . $manuscriptId
                    );
                }
            ...
            // Redirect - MUST happen before any HTML output
            $redirectUrl = '/jms/admin?action=submissions&subaction=' . ($status == 'submitted' ? 'new' : 'all');
...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-2xl font-bold text-[#0b2b3f]">Create New Submission</h2>
            <p class="text-gray-500 text-sm mt-1">Submit a new manuscript for review</p>
        </div>
        <a href="/jms/admin?action=submissions&subaction=new" class="text-indigo-600 hover:text-indigo-800 text-sm">
    ...
    <form method="POST" class="space-y-8" enctype="multipart/form-data">
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-6">
            ...
            <div class="grid md:grid-cols-2 gap-4">
                ...
                <div class="md:col-span-2">
                    ...
                    <p class="text-xs text-gray-400 mt-1">Upload a PDF file (Max 10MB). The PDF will be linked to the submission.</p>
                </div>
                <div class="md:col-span-2">
        ...
        <div class="flex gap-3">
            ...
            <a href="/jms/admin?action=submissions&subaction=new" class="bg-gray-100 text-gray-700 px-6 py-2.5 rounded-lg font-semibold hover:bg-gray-200 transition">
                Cancel
            </a>
        </div>
    </form>
